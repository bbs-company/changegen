import { ParsedCommit, CommitType } from './types';
import {
  COMMIT_TYPE_LABELS,
  COMMIT_TYPE_ORDER,
  groupCommits,
...
export function formatMarkdown(
  ...
  for (const type of COMMIT_TYPE_ORDER) {
    ...
    if (type === 'other' && typeCommits.every((c) => isNoise(c))) continue;
...
function formatMarkdownCommitLine(commit: ParsedCommit): string {
...
export function formatTerminal(
  ...
  for (const type of COMMIT_TYPE_ORDER) {
    ...
    lines.push(`${BOLD}${color}${COMMIT_TYPE_LABELS[type]}${RESET}`);
    for (const commit of typeCommits) {
...
function formatTerminalCommitLine(commit: ParsedCommit, _color: string): string {
